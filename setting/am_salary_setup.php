...
if(!isset($_SESSION['objLogin'])){
	...
	$emp_name ='';
	$designation = '';
    $defaultDesignation = "Amenity Manager";
	$month_id ='';
	$amount ='0.00';
	$issue_date = '';
	$branch_id = '';
	$button_text = $_data['save_button_text'];
	$form_url = WEB_URL . "setting/am_salary_setup.php";
...
if(isset($_POST['ddlEmpName'])){
		if($_POST['hdnSpid'] == '0'){
		$year = date('Y');
	$sql="INSERT INTO `tbl_add_am_salary_setup`(`emp_name`,`designation`,`month_id`,`xyear`,`amount`,`issue_date`,`branch_id`) VALUES ('$_POST[ddlEmpName]','$_POST[hdnDesg]','$_POST[ddlEmpMonth]',$year,'$_POST[txtEmpAmount]','$_POST[txtEmpIssueDate]','" . $_SESSION['objLogin']['branch_id'] . "')";	
			mysqli_query($conn,$sql);
			mysqli_close($conn);
		    $url = WEB_URL . 'setting/am_salary_setup.php?m=add';
		    header("Location: $url");
		}
else{
	
	$sql_update="UPDATE `tbl_add_am_salary_setup` set emp_name = '$_POST[ddlEmpName]',`designation`='".$_POST['hdnDesg']."',`month_id`='".$_POST['ddlEmpMonth']."',`amount`='".$_POST['txtEmpAmount']."',`issue_date`='".$_POST['txtEmpIssueDate']."' where emp_id= '" . (int)$_POST['hdnSpid'] . "'";	
			mysqli_query($conn,$sql_update,);
			mysqli_close($conn);
		    $url = WEB_URL . 'setting/am_salary_setup.php?m=up';
		    header("Location: $url");
...
if(isset($_GET['spid']) && $_GET['spid'] != ''){
		$result_location = mysqli_query($conn, "SELECT * FROM tbl_add_am_salary_setup where emp_id= '" . (int)$_GET['spid'] . "' and branch_id = '" . (int)$_SESSION['objLogin']['branch_id'] . "'");
		if($row = mysqli_fetch_array($result_location)){
			...
			$form_url = WEB_URL . "setting/am_salary_setup.php?id=".$_GET['spid'];
			$hval = $row['emp_id'];
		}
...
<section class="content-header">
  <h1><?php echo "Amenity Manager Salary Setup";?></h1>
  <ol class="breadcrumb">
  ...
  <div class="col-md-12">
    ...
    <div class="box box-info">
      <div class="box-header">
        <h3 class="box-title"><?php echo "Amenity Manager Salary Entry Form";?></h3>
      </div>
      <form onSubmit="return validateMe();" action="<?php echo $form_url; ?>" method="post" enctype="multipart/form-data">
        <div class="box-body">
        <div class="form-group">
        <label for="ddlEmpName"><?php echo "Amenity Manager Name";?> :</label>
        <select name="ddlEmpName" id="ddlEmpName" class="form-control">
        <option value=""><?php echo $_data['text_4'];?></option>
        <?php 
            $result_emp = mysqli_query($conn,"SELECT * FROM tbl_add_am order by eid ASC",$link);
            while($row_emp = mysqli_fetch_array($result_emp)){?>
            <option value="<?php echo $row_emp['eid'];?>"><?php echo $row_emp['e_name'];?></option>
        <?php } ?>
        </select>
        </div>
    <div class="form-group">
        <label for="txtEmpDesignation"><?php echo $_data['text_5'];?> :</label>
        <input readonly="readonly" type="text" name="txtEmpDesignation" value="<?php echo ($emp_name == '' || empty($emp_name)) ? $defaultDesignation : ''; ?>" id="txtEmpDesignation" class="form-control" />
        <input type="hidden" id="hdnDesg" name="hdnDesg" value="<?php echo $defaultDesignation; ?>" />
    </div>
          ...
          <div class="form-group pull-right">
			...
			&nbsp;
            <input type="reset" onClick="javascript:window.location.href='<?php echo WEB_URL; ?>setting/am_salary_setup.php';" name="btnReset" id="btnReset" value="<?php echo $_data['reset'];?>" class="btn btn-primary"/>
          </div>
        </div>
...
$msg = "";
 if(isset($_GET['delid']) && $_GET['delid'] != '' && $_GET['delid'] > 0){
		$sqlx= "DELETE FROM `tbl_add_am_salary_setup` WHERE emp_id = ".$_GET['delid'];
          ...
          <div class="box box-info">
            ...
            <div class="box-body">
              <table class="table sakotable table-bordered table-striped dt-responsive">
                ...
                <tbody>
                  <?php
				$result = mysqli_query($conn, "SELECT *,e.e_name,m.month_name FROM tbl_add_am_salary_setup es inner join tbl_add_am e on es.emp_name = e.eid inner join tbl_add_month_setup m on m.m_id = es.month_id where es.branch_id = " . (int)$_SESSION['objLogin']['branch_id'] . " order by es.emp_id ASC ");
...
<script type="text/javascript">
  function deleteEmployeeSalary(Id){
	...
	if(iAnswer){
		window.location = '<?php echo WEB_URL;?>setting/am_salary_setup.php?delid=' + Id;
	}
  }
  </script>
