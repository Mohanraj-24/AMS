        ...
        <div class="box box-info">
          ...
          <div class="box-body">
            <table style="font-size:13px;" class="table sakotable table-bordered table-striped dt-responsive">
              ...
              <tbody>
            <?php
			  $result = mysqli_query($conn, "select * from tbl_amenity_gym where g_id='".$_GET['gid']."'");
				while($row = mysqli_fetch_array($result))
                {
                  $res=mysqli_query($conn,"SELECT rent as r from tbl_add_amenity where amenity='Gym'");
...
<script>
  function deleteComplain(Id){
	...
	if(iAnswer){
		...
		window.location = '<?php echo WEB_URL; ?>t_dashboard/gym_pay.php?fid=' + Id;
	}
  }
  </script>
