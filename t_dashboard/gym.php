...
if(isset($_POST['txtCTitle']) && $_POST['txtCDate']){
    $d=$_POST['txtCDate'];
    $t=$_POST['txtCTitle'];
    $sql=mysqli_query($conn,"SELECT count(*) as count from tbl_amenity_gym where g_date='$d' and g_timezone='$t'");
    $countRow = mysqli_fetch_assoc($sql);
    $count = (int)$countRow['count'];
    if($count<=2)
    {
      if(isset($_POST['hdn']) && $_POST['hdn'] == '0'){
        $sqlx = "INSERT INTO tbl_amenity_gym(g_user_id,g_branch_id,g_date,g_timezone) values('".(int)$_SESSION['objLogin']['rid']."','" . $_SESSION['objLogin']['branch_id'] . "','$_POST[txtCDate]','$_POST[txtCTitle]')";
        mysqli_query($conn,$sqlx);
        $sqla=mysqli_query($conn,"SELECT g_id as gid from tbl_amenity_gym where g_user_id='".(int)$_SESSION['objLogin']['rid']."' and g_branch_id='" . $_SESSION['objLogin']['branch_id'] . "' and g_date='$_POST[txtCDate]' and g_timezone='$_POST[txtCTitle]'");
        $array=mysqli_fetch_assoc($sqla);
        $gid=(string)$array['gid'];
        mysqli_close($conn);
        $url = WEB_URL . 't_dashboard/gym_pay.php?gid='.$gid;
	      header("Location: $url");
      }
    }
    else{
...
<section class="content-header">
  ...
  <ol class="breadcrumb">
    ...
    <li class="active"><?php echo "Gym";?></li>
  </ol>
</section>
  ...
  <div class="col-xs-12">
    ...
    <div class="box box-info">
      <div class="box-header">
        <h3 class="box-title"><?php echo "Gym";?></h3>
      </div>
      <form onSubmit="return validateMe();" action="<?php echo WEB_URL?>t_dashboard/gym.php" method="post" enctype="multipart/form-data">
        <div class="box-body">
